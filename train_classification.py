...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', default= 'checkpoints/clf_vrvb3_5/m18/',                help='med_vqa_c$version$/m18/c80/m18_vid$temporal_size$/c80_vid$temporal_size$') #clf_v1_2_1x1/med_vqa_c3
    parser.add_argument('--dataset_type',   default= 'm18',                                         help='med_vqa/m18/c80/m18_vid/c80_vid')
    parser.add_argument('--dataset_cat',    default= 'cat3',                                        help='cat1/cat2/cat3')
    parser.add_argument('--transformer_ver',default= 'vrvb',                                        help='vb/vbrm/vrvb')
    parser.add_argument('--vrvb_ver',       default= 'v3',                                          help='V0,v1/v2/v3/v4')
    ...
    if args.dataset_type == 'med_vqa':
        ...
        # dataloader
        train_dataset = Cholec80VidVQAClassification(train_seq, folder_head, folder_tail, patch_size = args.patch_size, temporal_size=args.temporal_size)
        train_dataloader = DataLoader(dataset=train_dataset, batch_size= args.batch_size, shuffle=True)
        val_dataset = Cholec80VidVQAClassification(val_seq, folder_head, folder_tail, patch_size = args.patch_size, temporal_size=args.temporal_size)
        val_dataloader = DataLoader(dataset=val_dataset, batch_size= args.batch_size, shuffle=False)
    ...
    if args.checkpoint is None:
        ...
        elif args.transformer_ver == 'vrvb':
            model = ViReVisualBertClassification(vocab_size=len(tokenizer), layers=args.encoder_layers, n_heads=args.n_heads, num_class = args.num_class, version = args.vrvb_ver)
        # optimizer
